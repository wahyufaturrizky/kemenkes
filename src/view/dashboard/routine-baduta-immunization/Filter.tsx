    ...
    <div className="flex flex-wrap justify-between items-center gap-4 sm:mt-20 md:mt-0 mb-8">
      ...
      <div className="flex gap-4">
        <div
          onClick={async () => {
            const pct = data
              ?.filter((f: any) => f.label.includes("pct_"))
              ?.map((r: any) => r.value.toString());
            const total = data
              ?.filter((f: any) => f.label.includes("ytd_total_"))
              ?.map((r: any) => r.value.toString());
            const target = data
              ?.filter((f: any) => f.label.includes("threshold_"))
              ?.map((r: any) => r.value.toString());
            const header = data
              ?.filter((f: any) => f.label.includes("pct_"))
              ?.map((r: any) => r.label?.split("_")[1]);
    ...
    <div className="flex flex-wrap justify-between items-center gap-4 sm:mt-20 md:mt-0 mb-8">
      ...
      <div className="flex gap-4">
        <div
          onClick={async () => {
            const ideal = data
              ?.filter((f: any) => f.label.includes("ytd_ideal_"))
              ?.map((r: any) => r.value.toString());
            const nonIdeal = data
              ?.filter((f: any) => f.label.includes("ytd_non_ideal_"))
              ?.map((r: any) => r.value.toString());
            const header = data
              ?.filter((f: any) => f.label.includes("ytd_ideal_"))
              ?.map((r: any) => r.label?.split("_")[2]);
    ...
    <div className="flex flex-wrap justify-between items-center gap-4 sm:mt-20 md:mt-0 mb-8">
      ...
      <div className="flex gap-4">
        <div
          onClick={async () => {
            const male = data
              ?.filter((f: any) => f.label.includes("ytd_male_"))
              ?.map((r: any) => r.value.toString());
            const female = data
              ?.filter((f: any) => f.label.includes("ytd_female_"))
              ?.map((r: any) => r.value.toString());
            const header = data
              ?.filter((f: any) => f.label.includes("ytd_male_"))
              ?.map((r: any) => r.label?.split("_")[2]);
