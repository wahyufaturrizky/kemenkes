...
const FilterEarlyDetectionAnalysis: React.FC<FilterProps> = ({ filterState }) => {
      ...
      <div className="flex flex-wrap items-center gap-4">
        <div>
          <Select
            ...
            value={
              filter.tahun
                ? standardOptionSameLabel(generateYearsArray(1979, new Date().getFullYear()))?.find(
                    (f) => f.value === filter.tahun
                  )
                : filter.tahun
            }
          />
        ...
        <div>
          <Select
            ...
            value={
              filter.provinsi
                ? standardOptions(getProvince?.data || [], "provinsi_name", "provinsi")?.find(
                    (f) => f.value === filter.provinsi
                  )
                : filter.provinsi
            }
        ...
        <div>
          <Select
            ...
            value={
              filter.kabkota
                ? standardOptions(getRegency?.data || [], "kabkota_name", "kabkota")?.find(
                    (f) => f.value === filter.kabkota
                  )
                : filter.kabkota
            }
        ...
        <div>
          <Select
            ...
            value={
              filter.kecamatan
                ? standardOptions(getSubDistrict?.data || [], "kecamatan_name", "kecamatan")?.find(
                    (f) => f.value === filter.kecamatan
                  )
                : filter.kecamatan
            }
        ...
        {!filter.kewilayahan_type ? (
          ...
          <div>
            <Select
              ...
              options={standardOptions(getListFaskes?.data || [], "faskes_name", "faskes_id")}
              onChange={(e: any) => {
                setFilter({
                  ...filter,
                  faskes: e?.value,
              ...
              value={
                filter.faskes
                  ? standardOptions(getListFaskes?.data || [], "faskes_name", "faskes_id")?.find(
                      (f) => f.value === filter.faskes
                    )
                  : filter.faskes
              }
              isDisabled={!filter.kecamatan}
...
export default FilterEarlyDetectionAnalysis;
